...
namespace Alfred
{
  namespace EventManagerDebug
  {
    ...
    class ManagerDebug : virtual public Alfred::Utils::MakeFinal<ManagerDebug>
    {
      ...
      public:
        ...
        void addEvent(std::string name)
        {
          ...
          if (_events.count(name) > 0) {
            throw EventNameExistDebug(name); //todo remove this, put in private
          }
        ...
        std::vector<Ret> fireAndReturn(std::string name, const Args... args)
        {
          std::lock_guard<std::recursive_mutex> lock(_mutex);
          if (_events.count(name) <= 0) {
...
//                    throw EventDontExist(name);
        ...
        template <typename Ret, typename ...Args>
        void fire(std::string name, const Args... args)
        {
          std::lock_guard<std::recursive_mutex> lock(_mutex);
          if (_events.count(name) <= 0) {
            addEvent<Ret, Args...>(name);
//                    throw EventDontExist(name);
        ...
        const EventListener listen(std::string name, const Fctor &func)
        {
          std::lock_guard<std::recursive_mutex> lock(_mutex);
          if (_events.count(name) <= 0) {
...
//                    throw EventDontExist(name);
        ...
        template <typename Ret, typename ...Args>
        void unlisten(std::string name, const EventListener listenerID)
        {
          std::lock_guard<std::recursive_mutex> lock(_mutex);
